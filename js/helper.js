...
//From JavaScript Basics Course:
/* function locationizer(work_obj){ //For loop
  
  var myArray=[];
  for(var i=0; i<work_obj.jobs.length; i++){
  
  myArray.push(work_obj.jobs[i].location);

  }
   return myArray;
}

// function locationizer(work_obj){ //For in loop
  var myArray=[];
  for(job in work_obj.jobs){
  
  var jobLocation= work_obj.jobs[job].location;

  myArray.push(joblocation);
  }
